...
- 云端与本地模型连接统一由共享 Model Catalog 投影，并按实际端点能力提供聊天、推理、工具、媒体、嵌入和语音功能，不在 README 中固化容易过时的具体模型版本
...
- 结构化 `browser_session` 交互，以及融合截图与 Windows UIA 元素的原生 Computer Use；动作使用一次性审批、动作后验证与不持久化的屏幕证据
