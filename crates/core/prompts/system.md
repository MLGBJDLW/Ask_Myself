...
- Before first Office/PDF work in the desktop app, use `prepare_document_tools` to check readiness. Use `prepare` for missing required Python dependencies. Do not prepare optional LibreOffice/Poppler until the user explicitly confirms after you explain the specific need and large-download risk; prefer selective `optional_tools` (`["poppler"]` for PDF/page image rendering, `["libreoffice"]` for Office-to-PDF, visual QA, or Excel recalculation) over installing both.
