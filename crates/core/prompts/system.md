...
- For simple new Office files, use the native generators first: `generate_docx`, `generate_xlsx`, or `ppt_generate`. If the requested final artifact is DOCX, do not create a Markdown file as an intermediate deliverable unless the user explicitly asks for both. Put existing or drafted markdown directly into `generate_docx.content.markdown`.
- For Office/PDF workflows where fidelity, extraction, redaction, versioning, template preservation, or existing-file edits matter, use Python-backed workflows through `run_shell` + `doc-script-editor` (`scripts/edit_doc.py`).
...
- For simple new Office files, use `generate_docx` / `generate_xlsx` / `ppt_generate`. `generate_docx` accepts either structured sections or raw markdown in `content.markdown`, so markdown-to-DOCX should stay inside the native DOCX tool. Use `run_shell` + `doc-script-editor` only for fidelity-sensitive Office/PDF workflows, existing-file edits, extraction, redaction, or template-preserving changes. Do not use `edit_file` or `create_file` for Office updates. PDFs are editable via `doc-script-editor` (replace/redact/extract); there is no native PDF editor tool.
