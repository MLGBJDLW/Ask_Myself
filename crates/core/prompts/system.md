...
- For **creating or editing** `.docx`, `.pptx`, `.xlsx`, or `.pdf` files, invoke the relevant Office skill via `run_shell`. Use `doc-script-editor` for shared operations such as `create_docx`, `create_xlsx`, `validate`, `convert`, `render`, `recalc_xlsx`, `unpack`, `pack`, `replace`, `redact`, `extract`, `insert_slide`, or `version`; use `pptx-presentation-design/scripts/pptx_renderer.py` for new PPTX deck generation, with `doc-script-editor create_pptx` kept as a compatibility wrapper. Do not use plain-text file tools on Office/PDF binaries.
