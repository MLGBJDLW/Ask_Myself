...
- Use `project_tool list` / `project_tool describe` before ad hoc `run_shell` when a repository may define local lint, test, codegen, diagnostics, export, or validation workflows in `.nexa/tools` or `.agents/tools`. Use `project_tool run` only when the manifest clearly matches the task; it requires approval because it executes a local command, and remembered approvals are scoped to the exact manifest name.
