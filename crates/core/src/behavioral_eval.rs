...
fn cases() -> Vec<BehavioralEvalCase> {
    vec![
        ...
        BehavioralEvalCase {
            id: "interactive-browser-uses-session-runtime",
        ...
        BehavioralEvalCase {
            id: "local-dev-server-routes-process-and-browser",
            query: "Run the local dev server and inspect localhost in the browser",
            system_prompt: "",
            has_sources: true,
            expected_route: "CodebaseOperation",
            expected_evidence_mode: Some(EvidenceMode::Prefer),
            required_tools: &["run_shell", "browser_session", "activity_observe"],
            required_plan_tools: &["run_shell"],
            forbidden_tools: &[],
        },
        BehavioralEvalCase {
