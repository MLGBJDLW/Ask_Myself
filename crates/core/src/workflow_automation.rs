...
use crate::conversation::{AgentTaskArtifact, AgentTaskArtifactSummary, AgentTaskRun};
...
impl Database {
    ...
    pub fn create_task_resume_checkpoint_with_state(
        ...
        drop(conn);
        self.get_task_resume_checkpoint(&checkpoint_id)
    }

    pub fn pause_task_run_with_checkpoint(
        &self,
        run_id: &str,
        reason: &str,
    ) -> Result<TaskResumeCheckpoint, CoreError> {
        let checkpoint = self.create_task_resume_checkpoint(run_id, reason)?;
        self.update_agent_task_run_progress(
            run_id,
            Some("paused"),
            Some("paused"),
            None,
